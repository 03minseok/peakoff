 ...
 *       Map 조회만 하므로 <b>요청마다 새로 센다</b>. 창이 이레에서 예측 전체(24~30일)로
 *       넓어졌지만 여전히 Map 조회라, 늘어난 것은 조회 횟수뿐이다</li>
 * </ul>
...
public class KtoRegionProfileProvider implements RegionProfileProvider {
	...
	private RegionProfile profileOf(SupportedRegion supported, LocalDate from, int days) {
		...
		Window window = windowOf(congestionClient.forecastOf(region), from, days);

		return new RegionProfile(supported, shares, window.quietShare(), window.places());
	}
	...
	/**
	 * 창 안의 예측에서 <b>한적한 관측이 몇 할인지</b>.
	 *
	 * <p>평균이 아니라 비율인 이유는 {@link RegionProfile}에 적어 두었다 —
	 * 11곳의 평균이 48~63으로 전부 "보통"이라, 평균으로는 카드가 서로 구분되지 않는다.
	 *
	 * <p>⚠️ <b>창이 넓어지면 값이 함께 내려간다.</b> 이레만 볼 때 21.6~64.7%였던 것이
	 * 30일에서는 16.6~51.6%다(2026-09-05 실측) — 붐비는 날이 더 섞이기 때문이지
	 * 지역이 더 붐벼서가 아니다. 지역 간 폭(43 → 35)과 순위는 살아 있고,
	 * 카드가 모수를 함께 적으므로 <b>내려간 값도 거짓이 되지 않는다</b>.
	 *
	 * <p>모수는 <b>(예측 대상 × 날짜)의 관측 전체</b>다. 장소마다 가장 한적한 날만 골라
	 * 세는 방법도 재 봤는데(39~96%) 위쪽에 몰려 지역이 갈리지 않았다 —
	 * <b>가장 좋은 날만 보면 어디나 한적하다.</b>
	 */
	private static Window windowOf(RegionForecast forecast, LocalDate from, int days) {
		...
		if (observed == 0) {
			// 지역은 있는데 창에 걸치는 날짜가 없다. 자료 없음과 같이 다룬다.
			return new Window(null, 0);
		}
		return new Window((int) Math.round(100.0 * quiet / observed), places);
	}

	/** 창 요약. 비율과 모수를 함께 든다 — 비율만 내걸면 몇 곳을 본 값인지 알 수 없다. */
	private record Window(Integer quietShare, int places) {
	}
}
