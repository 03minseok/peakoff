...
public class SecurityConfig {
	...
	private static final String[] PUBLIC_API = {
			...
			"/api/regions",
			/*
			 * 오늘 공사 API를 몇 번 불렀나(읽기 전용 집계). 호출 이력이 공모전 규칙 1의 증거라
			 * 심사에서 화면으로 보여줄 값이고, 개인과 무관한 숫자라 감출 이유가 없다.
			 */
			"/api/quotas",
			/*
			 * 공유 링크로 여는 코스. 토큰이 열쇠라 로그인이 없다 — 받은 사람이 게스트여도 봐야 링크다.
			 * /api/courses의 나머지는 여전히 로그인 필수다(이 한 갈래만 연다).
			 */
			"/api/courses/shared/*",
			/*
			 * 여행지 추천 챗봇. "어디 갈까"는 여행 계획의 첫 결정이라 로그인 뒤에 두면
			 * 아무도 만나지 못한다. 회원이면 토큰이 함께 오고, 그때는 IP가 아니라
			 * 사용자 ID로 호출량을 센다.
			 */
			"/api/chat/**",
