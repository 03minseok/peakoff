 ...
 * 교체 추천은 후보를 어디서 가져왔는지를 말하고({@link CandidateSource#noteFor} —
 * "불국사에 다녀간 사람들이 함께 찾은 곳 중에서 골랐어요"), 설문 코스 생성은 동선을 말한다
 * ({@code CourseDraftService.reasonFor} — "하루를 시작하는 곳" · "불국사에서 4.2km").
 * 계산은 한 곳에서 하되 말하는 방식은 화면마다 다르므로, 그 경계를 타입으로 갈라 뒀다.
 *
 * <p>⚠️ 두 문구 <b>어느 쪽도 혼잡을 말하지 않는다</b>(2026-09-01에 걷어냈다). 한때 둘 다
 * "· 예상 혼잡 낮음"으로 끝났는데, 바로 윗줄에 한적도 배지가 이미 서 있어 같은 사실을
 * 숫자로 한 번 말로 한 번 말하는 셈이었다.
 *
