 ...
 * </ul>
 *
 * <p>점수가 <b>아닌</b> 것이 둘 있다. 둘 다 후보를 고르는 쪽의 <b>필터</b>다.
 * <ul>
 *   <li><b>연관성</b>("함께 많이 방문되는 곳") — ⚠️ <b>점수 항목으로 넣지 않는다</b>
 *       (2026-08-31 확정). 한때 여기 "연관 관광지 데이터가 붙으면 항목이 하나 늘어난다"고
 *       적어 두었는데, 데이터를 붙여 재 봤더니 <b>넣으면 안 되는 값</b>이었다 —
 *       연관 순위와 한적도가 음의 상관이라(6개 지역 26,819쌍, 켄달 타우 -0.073)
 *       가점을 주면 더 붐비는 곳을 더 미는 셈이 된다. 자세한 것은 {@link ScoreWeights}.
 *       후보를 만드는 문으로만 쓰고, 그 사실은 {@link CandidateSource}가 문장으로 말한다.</li>
 *   <li><b>카테고리 적합성</b> — {@code PlaceCategories.compatible}이 중분류까지 보고 거른다.</li>
 * </ul>
 *
