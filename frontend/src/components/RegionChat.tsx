    ...
    <div className="flex min-h-0 flex-1 flex-col gap-3">
      ...
      */}
      <div className="bg-bg flex min-h-0 flex-1 flex-col overflow-hidden rounded-[18px]">
        ...
        {/*
          입력 막대. 메신저처럼 <b>바닥에 흰 띠</b>로 앉는다.
          ⚠️ 화면 바닥이 아니라 <b>카드 안쪽 바닥</b>이다 — position:fixed를 쓰지 않으므로
          모바일 브라우저의 도구막대와 자리를 다투지 않는다(CLAUDE.md).
        */}
        <div className="bg-surface border-line/70 flex flex-col gap-2 border-t p-2.5">
          {/*
          ...
          */}
          ...
          <form onSubmit={submit} className="flex items-center gap-2">
            <input
              ...
              aria-label="여행지 질문"
              className="bg-fill text-fg placeholder:text-hint h-10 min-w-0 flex-1 rounded-full border-none px-4 text-[13.5px]"
            />
            {/*
