...
export function Layout() {
  ...
  return (
    <div className="flex min-h-svh flex-col">
      ...
      {/*
        ■ 출처 한 줄 — 껍데기가 든다 (2026-09-09)

        공모전 규정이 데이터 출처 표기를 <b>필수</b>로 두는데, 이 줄이 화면마다 따로 있었다
        (홈·진단·결과·장소 상세). 그러면 없는 화면이 생긴다 — 코스 편집·코스 발견·마이페이지가
        그랬다. 셋 다 공사 데이터로 그려지는 화면인데 출처가 없었다.

        <p>Layout 아래 화면은 <b>전부</b> 공사 데이터를 쓴다(지역 카탈로그·집중률·저장 코스의
        장소). 그래서 화면마다 문자열을 박는 대신 껍데기가 한 번 든다 — 빠질 자리가 없다.
        홈·로그인·가입은 Layout 밖이다: 홈은 자기 줄이 있고, 계정 화면은 공사 데이터를 안 쓴다.

        <p>같은 문제를 OffWay(team-offway/core)는 응답 DTO가 기관 집합을 선언하는
        {@code Attributed}로 풀었다. 그쪽은 기관이 여섯이라 그래야 했고, 우리는 하나라
        그 장치가 상수를 매 응답에 실어 보내는 과잉이 된다. 목적은 같다 — "가끔 빠짐"이
        허용되지 않는 표기를 <b>구조</b>로 지킨다.

        <p>형식은 공고 FAQ 그대로 {@code 출처: ⓒ한국관광공사}(콜론). 텍스트만 — 공사 CI/BI
        이미지는 쓸 수 없다. {@code TourAPI}처럼 API 이름 단독 표기는 지양.
      */}
      <footer className="max-w-app text-hint mx-auto w-full px-4.5 pb-6 text-center text-[11px] md:px-6 lg:px-8">
        출처: ⓒ한국관광공사
      </footer>
    </div>
  )
