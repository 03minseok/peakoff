 ...
 * 사슬 고리 — "링크".
 *
 * <p>⚠️ 이름에 {@code Icon}이 붙은 유일한 아이콘이다. 여러 화면이 react-router의 {@code Link}를
 * 쓰고 있어, 같은 이름이면 한 파일에서 둘을 함께 부를 수 없다. 이름이 부딪히는 자리를
 * 만들지 않으려고 여기서만 접미사를 단다.
 *
 * <p>고리 둘을 <b>각각 한 획</b>으로 그린다 — 가운데가 겹쳐 보이도록 반대편으로 조금씩
 * 넘어가게 두면, 두 도형을 따로 그리지 않아도 이어진 사슬로 읽힌다.
 */
export function LinkIcon({ size = 18, className = '' }: Props) {
  return (
    <svg {...ICON} width={size} height={size} className={className}>
      <path d="M8.33 10.83a4.17 4.17 0 0 0 6.29.45l2.5-2.5a4.17 4.17 0 0 0-5.89-5.89l-1.44 1.43" />
...
/**
 * 지도 — <b>세 폭으로 접힌 종이 지도</b>.
 *
 * <p>{@link Route}(길)와 뜻이 다르다. 저쪽은 "순서대로 이은 여러 곳"이고, 이것은
 * <b>그것을 펼쳐 보는 물건</b>이다. 저장된 여행 시트의 "코스 동선 보기"가 쓴다 —
 * 누르면 열리는 것이 경로가 아니라 지도라서, 문패에는 지도가 선다.
 *
 * <p>접힌 자리 두 줄이 폭을 셋으로 가른다. 가르는 선이 없으면 그냥 사각형이라
 * 이 크기에서는 사진 액자와 구분되지 않는다.
 */
export function MapIcon({ size = 20, className = '' }: Props) {
  return (
    <svg {...ICON} width={size} height={size} className={className}>
      <path d="M2.5 5.2 7.5 3.2 12.5 5.2 17.5 3.2v11.6l-5 2-5-2-5 2z" />
      <path d="M7.5 3.2v11.6M12.5 5.2v11.6" />
    </svg>
  )
}
