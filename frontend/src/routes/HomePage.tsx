 ...
 * 요청 수가 후보 수에 가까워지면 "다 가져가라"와 같아져 무작위가 고를 것이 없어진다 —
 * 2026-08-26에 대안 시트가 여덟을 요청해 분산 장치를 죽인 것이 그 모양이었다.
 * 여기서도 <b>지역 대표는 지역 수까지</b>라 다섯이면 위험해 보였다.
 *
 * <p>재보니 아니었다(12회 호출, 실데이터). 서버가 <b>지역 대표를 요청마다 새로 뽑기</b>
 * 때문에 후보 공간이 지역 수가 아니라 (지역 × 그 지역의 한적한 곳들)이다.
 *
 * <p>2026-09-03에 서버가 그 후보 공간을 <b>지역 상위 35%</b>로 넓혔다(예전에는 지역마다
 * 상위 세 곳이 전부였다). 다섯을 요청해도 지역마다 수십 곳에서 고르게 뽑히므로
 * 이 값은 그대로 둔다. <b>열 이상으로 올릴 때는 다시 재야 한다</b> — 그때 걸리는 것은
 * 후보 수가 아니라 <b>지역 수</b>다. 지역마다 한 곳씩만 세우기 때문이다.
 */
 ...
 * 이 목록은 <b>지원 지역을 한데 섞은 것</b>이라, 어느 카드가 어디인지 카드가 스스로
  ...
  return (
    ...
    <div className="relative flex min-h-svh flex-col pb-10">
      {/*
        ...
        잉크 글자가 얹혀 대비가 무너진다(로고·진입 카드의 제목).
      ...
      */}
      <div className={`${SHELL} relative px-4.5 md:px-6 lg:px-8 lg:pt-8`}>
        <div className="flex flex-col gap-4 lg:grid lg:grid-cols-12 lg:gap-4">
          {/*
            ■ <b>화면이 자기소개를 하지 않는다</b> (2026-09-08에 걷어냈다)

            모바일 첫 줄에 <b>"오늘, 어떤 여행을 원하세요?"</b>가 있었다. 진입 카드 둘이
            사용자가 하는 말("가고 싶은 곳이 있어요")이라, 서비스가 무엇을 하는 곳인지
            말하는 줄이 하나도 없다는 이유로 세웠던 것이다(2026-09-02).

            <p>걷어낸 이유는 <b>그 아래가 이미 말하고 있어서</b>다. 진입 카드 둘에 사진이
            들어오면서 화면이 무슨 이야기를 하는 곳인지 글을 읽기 전에 드러났고, 물음 한 줄은
            그 위에서 <b>카드가 할 말을 미리 하는</b> 군더더기가 됐다.

            <p>⚠️ 되살리려거든 <b>모바일에만</b> 세울 것. lg에서는 진입 카드 둘이 한 줄에
            나란히 서서 첫 화면이 이미 꽉 차 있다.
          */}
        ...
        */}
        ...
        {/*
          ...
          반대로 갔다. 지역을 고르는 대신 <b>전 지역을 한 번에 훑어</b> 이번 주 한적할 곳을
