          ...
          <span className="relative flex flex-col gap-3">
            ...
            */}
            ...
            {/*
              ...
              <p>⚠️ <b>그래서 본문의 "~라면" 절을 지우지 말 것.</b> 지금 두 문을 가르는 일은
              제목이 아니라 그 절이 한다. 특히 옆 카드의 "어디 갈지 모르겠다면"은
              경주를 모르는 사용자를 위한 문패다 — 그것이 없으면 그 사람은 빈 검색창 앞에서
              처음 막힌다(CLAUDE.md 필수 기능 6번이 이 진입점을 두는 이유).
            */}
            <span className="text-[26px] leading-[1.3] font-bold tracking-[-0.025em]">
              코스 짜기
            </span>
            {/*
              첫 절이 <b>길 안내</b>다(위 주석 참고). "있다면"이라 단언이 아니고,
              그래서 어떤 사용자에게도 틀리지 않으면서 자기 쪽인지 알려준다.

              뒷절 "계획은 그대로"는 이 카드만의 말이 아니다 — 진단 화면의 두 회피 경로
              ("일정은 그대로, 더 여유로운 날을" · "계획은 그대로, 더 여유로운 여행지를")와
              같은 문형이라, <b>홈에서 시작한 약속이 그 화면들에서 그대로 지켜진다.</b>
            ...
            */}
            <span className="max-w-62.5 text-sm leading-[1.6] text-white/60">
              가고 싶은 곳이 있다면.<br/>계획은 그대로, 붐비는 순간만 PEAKOFF가 도와드려요.
            </span>
          ...
          <div className="group border-brand bg-surface shadow-rest relative w-full overflow-hidden rounded-[24px] border-[1.5px] px-6 pt-6.5 pb-6 text-left transition-[transform,box-shadow] duration-200 hover:-translate-y-1 hover:shadow-raised motion-reduce:transition-none motion-reduce:hover:translate-y-0 lg:flex-1 lg:px-8 lg:pt-9">
            <span className="relative flex flex-col gap-3">
              ...
              */}
              ...
              {/*
                ⚠️ 첫 절 "어디 갈지 모르겠다면"이 <b>경주를 모르는 사용자를 위한 문패</b>다.
                제목이 행동 이름이 되면서 길 안내를 통째로 이 절이 맡는다 — 지우면 그 사람은
                왼쪽 문으로 들어가 빈 검색창 앞에서 처음 막힌다.
              */}
              <span className="text-muted max-w-62.5 text-sm leading-[1.6]">
                어디 갈지 모르겠다면.<br/>날짜와 취향만 알려주면 몰랐던 여행지를 PEAKOFF가 찾아드려요.
              </span>
              {/*
