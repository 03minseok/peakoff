            ...
            */}
            ...
            {/*
              ■ 두 카드의 제목은 <b>사용자의 상황</b>을 묻는다 — "당신은 어느 쪽인가"
                ...
                이미 계획이 있어요    → 그 계획을 넣고 진단받는다
                아직 계획이 없어요   → 두 문항 답하면 코스가 나온다

              ⚠️ "내가 고른 여행"이었다가 고쳤다. <b>시제가 틀렸다</b> — 누르기 전에 읽는
              문구인데 이미 "고른"(과거)이라 말했다. 상황으로 나누면 시제가 틀릴 수 없고,
              읽는 즉시 어느 문으로 들어갈지가 정해진다.

              <h3>⚠️ "가고 싶은 곳이 있어요"에서 한 번 더 옮겼다 (2026-08-29)</h3>
              그 문구는 <b>필요한 것을 실제보다 적게 말했다.</b> 이 문을 지나면 지역·날짜·기간을
              넣고 장소까지 일자별로 다 담아야 진단이 돈다 — 필요한 것은 "곳" 몇 개가 아니라
              <b>계획 한 벌</b>이다. 곳만 있으면 된다고 말해 놓고 들어온 뒤에 날짜와 일정을
              요구하면, 문턱을 낮춘 것이 아니라 뒤로 미룬 것이다.

              <p>두 카드를 실제로 가르는 것도 날짜가 아니다 — <b>옆 카드도 날짜는 받는다.</b>
              갈리는 것은 "장소를 내가 정했는가"이고, 그것을 담는 말이 "계획"이다.

              <h3>⚠️ 갈림길은 제목이 짓고, 갈래를 정확히 하는 일은 본문이 한다</h3>
              두 제목이 <b>같은 낱말의 있다/없다</b>라 읽는 즉시 자기 쪽이 정해진다.
              대신 그만큼 거칠어진다 — "경주에 9월 16일에 간다, 다만 어디 갈지는 모른다"는
              사람은 <b>계획이 있다고 여겨 이쪽을 고를 수 있다.</b> 그러면 빈 검색창 앞에서
              처음 막히는데, 설문 진입점은 원래 그 사람을 위한 문이다.

              <p>그래서 <b>본문이 갈래를 마저 짓는다.</b> 옆 카드가 "날짜와 취향만 알려주세요 ·
              몰랐던 여행지를 찾아드려요"라고 말해 그 사람을 도로 데려간다. 안전망도 둘 더 있다 —
              {@code PlanPage}의 "어디 갈지 아직 모르겠다면?" 링크와, 코스 편집 화면이
              빈 검색창에 세워 두는 대표 관광지 칩이다.

              <p>⚠️ 그러니 <b>옆 카드의 본문에서 "몰랐던 여행지를 찾아드려요"를 빼지 말 것.</b>
              제목만 남기면 이 갈림길이 헐거워진다.
            */}
            <span className="text-[26px] leading-[1.3] font-bold tracking-[-0.025em]">
              이미 계획이 있어요
            </span>
            {/*
              제목과 <b>같은 낱말</b>로 받는다. 제목이 "계획"인데 본문이 "가고 싶은 곳"이면
              한 카드가 두 말을 하게 된다.

              "계획은 그대로"는 이 카드만의 말이 아니다 — 진단 화면의 두 회피 경로
              ("일정은 그대로, 더 여유로운 날을" · "계획은 그대로, 더 여유로운 여행지를")와
              같은 문형이다. <b>홈에서 시작한 약속이 그 화면들에서 그대로 지켜진다.</b>
              서비스가 하는 일이 여행을 대신 정하는 것이 아니라 붐비는 부분만 비껴 주는
              것이라, 두 진입 카드 다 "당신 것은 그대로 둔다"로 시작한다.
            */}
            ...
            >
              {/*
                제목이 "이미 계획이 있어요"이므로 버튼도 그 계획을 받는 말이어야 한다.
                "짜기"라고만 하면 계획이 있다고 답한 사람에게 <b>처음부터 다시 짜라</b>는
                말로 들린다 — 실제로 하는 일은 머릿속 계획을 화면에 옮기는 것이다.
              */}
              내 계획으로 시작하기
