...
잠시 후 다시 시도해 주세요.`,
  ...
  return (
    <AuthShell
      ...
      */
      /*
        ⚠️ <b>맺음이 있어야 한다</b> (2026-09-09). "알려드리고, 저장해두고"로 두 번 이어놓고
        끝내지 않아, 뒤에 한 줄이 잘려 나간 것처럼 보였다. 가입 화면 제목은
        "계정에 담아두세요"로 끝나는데 이쪽만 열려 있었다.
      */
      panelTitle={
        <>
          붐비는 곳은 미리 알려드리고,
      ...
      /*
        ⚠️ <b>두 문장을 한 문장으로 줄이지 말 것.</b> 같은 사실을 앞뒤로 말하는 것처럼
        보이지만 하는 일이 다르다 — 앞은 무엇이 필요한지, 뒤는 <b>없어도 된다</b>는 것을
        말한다. 게스트가 로그인 화면에서 가장 걱정하는 것이 뒤쪽이라, 줄이면 안심시키는
        말이 사라진다.

        ⚠️ <b>"혼잡도"가 아니라 "예상 혼잡"이다.</b> 공사 자료는 예측이라 그냥 혼잡도라
        하면 지금 붐비는 정도를 보여주는 것처럼 읽힌다(CLAUDE.md 절대 규칙 아래 ⚠️).
        화면이 이 값을 부르는 이름도 "한적 지수"·"예상 혼잡"이지 혼잡도가 아니다.

        ⚠️ <b>해요체다.</b> "가능합니다"였다. 이 저장소가 합쇼체를 안 쓰는 것은 아니지만
        <b>쓰는 자리가 정해져 있다</b> — 실패("불러오지 못했습니다"), 경고("계정에서 완전히
        사라집니다"), 법적 확인("만 14세 이상입니다"), 결과 진술("원안 그대로입니다")이다.
        말을 거는 안내는 줄곧 해요체다. 이 문장은 <b>안심시키는 안내</b>라 그쪽에 속하는데,
        바로 앞 문장이 "필요해요"인데도 혼자 합쇼체로 서 있었다.
      */
      panelDescription={
        '로그인은 코스를 저장할 때만 필요해요.\n계획을 짜고 예상 혼잡을 보는 건 계정 없이도 전부 할 수 있어요.'
      }
    >
      ...
      <div className="flex flex-col gap-2.5 pt-3 pb-6">
        ...
        {/*
          ⚠️ <b>"어느 기기에서든"이었다</b> (2026-09-09). 사실이지만 계정이 해주는 일 중
          가장 곁가지다 — 옆 가입 화면이 같은 자리에서 저장과 견주기를 말하는데 여기만
          기기를 말해, 두 화면이 계정을 <b>다른 물건</b>으로 소개하고 있었다.

          <p><b>좁은 화면에서는 이 줄이 유일한 설명이다.</b> 왼쪽 패널이 lg부터만 서므로
          지우면 안 되고 바꿔야 한다. 넓은 화면에서 패널과 겹쳐 보이는 것은 그 대가다.
        */}
        <p className="m-0 text-[14.5px] leading-[1.6] break-keep text-pretty">
          로그인하면 저장해 둔 코스를 다시 열어 이어서 짤 수 있어요.
        </p>
      </div>
